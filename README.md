...
| クラスタ | ノード | 役割 | 補足 |
|----------|--------|------|------|
| `natsume` | `natsume-03`, `natsume-08` | 本番ワークロード、監視基盤、DB | `natsume-03` が external etcd と K3s server を持ち、`natsume-08` が K3s agent、TopoLVM、Longhorn 用ストレージを持つ |
...
natsume では `natsume-08` が storage node です。
`topolvm_storage` は `/dev/vda4` に 300GiB の partition を作り、VG `topolvm` を作成します。
`longhorn_storage` は `/dev/vda5` を使い、LV `data` を `/var/lib/longhorn` に ext4 で mount します。
